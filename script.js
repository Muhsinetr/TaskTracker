...
//color array
let myColor = ["#F9ACA7","#FFD767","#8cec55","#f55159","#a2aa90","#6896ec","#c36dfc","#f06e99","#baf796","#7df0f8"];
...
function tagToShow(e){
    ...
    let Listarray = {Task:{Name:[],description:[],List:[],Date:[],status:[]}}
...
function setMenuItemNum(){
    ...
    document.querySelector("#Stickywall div").innerHTML = mainArray.StickyWall.Text.length;
}
...
function swSave(){
    ...
    if(swtextarea.value){
        ...
        mainArray.StickyWall.Text.push(swtextarea.value.charAt(0).toUpperCase() + swtextarea.value.slice(1));
...
function deleteSw(e){
    ...
    if(e.target.className ==='clr'){
        let clrInd = e.target.parentElement.parentElement.parentElement.previousElementSibling.previousElementSibling.innerText;
...
display.addEventListener("click", function (e) {
    ...
    } else if (e.target.classList[0] === "arrow" && HideShow.style.display !== 'none') {
    ...
    if (e.target.classList[0] === "dltbtn" && confirm("Are you sure?") == true) {
        let checkName = e.target.parentElement.parentElement.previousElementSibling.children[1].innerHTML;
...
listText.addEventListener("keypress",function(event){if(event.key==="Enter"){addList();}});
...
swtextarea.addEventListener("keypress",function(event){if(event.key==="Enter"){swSave()}});
